...
lidar = UART(
    0, baudrate=115200, tx=Pin(16), rx=Pin(17)
...
def set_samp_rate(samp_rate=frame_rate):
    ...
    print(
        "hex_rate:{}".format(hex_rate)
    )  # \xHH\xLL => hex_rate[0]=\xHH hex_rate[1]=\xLL
    samp_rate_packet = [
        0x5A,
        0x06,
        0x03,
...
def getLidarData(UART0):
    ...
    if UART0.any() > 0:
        ...
        if bin_ascii[0] == 0x59 and bin_ascii[1] == 0x59:
            ...
            temperature = (
                bin_ascii[6] + bin_ascii[7] * 256
            ) / 8 - 256  # Get IC temperature value
            """
