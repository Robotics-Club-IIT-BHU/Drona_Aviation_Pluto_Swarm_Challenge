...
class cartesian_cnt:
    ...
    def __init__(self,g: float=9.8):
        ...
        '''
        self.P_COEFF_FOR = np.array([.1, .1, .2])               # Proportional Coefficient
        self.I_COEFF_FOR = np.array([.0001, .0001, .0001])      # Integrate Coefficient
        self.D_COEFF_FOR = np.array([.3, .3, .4])               # Derivative Coefficient
    ...
    def _simplePIDPositionControl(self,
        ...
        self.integral_pos_e = self.integral_pos_e + pos_e*self.control_timestep     # Updating integral error
        target_force = np.array([0, 0, self.GRAVITY]) \
                       + np.multiply(self.P_COEFF_FOR, pos_e) \
                       + np.multiply(self.I_COEFF_FOR, self.integral_pos_e) \
                       + np.multiply(self.D_COEFF_FOR, d_pos_e)                     # Calulating Target force using PID controller
